...
class _SettingsPageState extends State<SettingsPage> {
      ...
      child: ListView(
       children:  ListTile.divideTiles(context: context,color: Colors.black,
         tiles: [
           ...
           Padding(
             padding: const EdgeInsets.all(8.0),
             child: ListTile(title: Text("Change Email"),trailing: Icon(Icons.arrow_forward_ios, color: Colors.black,),onTap:() {
               mailDialog(context, auth);
             }),
           ),
           Padding(
             padding: const EdgeInsets.all(8.0),
             child: ListTile(title: Text("Change Password"),trailing: Icon(Icons.arrow_forward_ios, color: Colors.black,),onTap:() {
  ...
  void mailDialog(BuildContext context,AuthService auth) {
    showDialog(context: context, builder: (context){
      return SimpleDialog(
        title:Text("Change Email", style: TextStyle(color: Theme.of(context).primaryColor),),
        children: [
          Padding(
            padding: const EdgeInsets.all(16.0),
            child: Form(
